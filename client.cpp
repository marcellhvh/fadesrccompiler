...
void Client::DrawHUD() {
	...
	//std::string text = tfm::format(XOR("fade | delay: %ims | tick: %i | %s"), ms, rate, time.str().data());
...
void Client::ClanTag()
{
	...
	auto SetClanTag = [&](std::string tag) -> void {
		...
		SetClanTagFn(tag.c_str(), XOR("fade"));
	};

	std::string szClanTag = XOR("fade");
	...
	if (g_menu.main.misc.clantag.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 3.6) % 18) {
			...
			case 9: {  SetClanTag("fade"); break; }
			case 10: { SetClanTag("fade"); break; }
			case 11: { SetClanTag("fade"); break; }
...
void Client::UpdateLocal( )
{
	...
	if ( !g_cl.m_local || !g_cl.m_processing || !g_csgo.m_net )
		return;
	...
	if ( state->m_last_update_frame >= v4 )
		state->m_last_update_frame = v4 - 1;
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf( colors::green, XOR( "[fade] " ) );
	g_csgo.m_cvar->ConsoleColorPrintf( Color( 128, 128, 128 ), buf.c_str());
}
