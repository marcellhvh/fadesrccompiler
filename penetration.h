...
namespace penetration {
	...
	bool  TraceToExitShort(vec3_t& point, vec3_t& dir, const float step_size, float max_distance);
	void  TraceLine(const vec3_t& start, const vec3_t& end, uint32_t mask, ITraceFilter* ignore, CGameTrace* ptr);
	float get_thickness(vec3_t& start, vec3_t& end, const float distance);
