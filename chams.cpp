...
bool Chams::GenerateLerpedMatrix(int index, BoneArray* out) {
	...
	if (data->m_records.front().get()->brokelc())
		return false;
...
void Chams::RenderPlayer(Player* player) {
	...
	// we always draw the local player manually in drawmodel.
	if (player->m_bIsLocalPlayer()) {
		if (g_menu.main.players.chams_local_scope.get()) {
			if (player->m_bIsScoped() || g_cl.m_weapon_type == WEAPONTYPE_GRENADE)
				g_visuals.animation[37] -= 1 * (g_csgo.m_globals->m_frametime * 5);
			else
				g_visuals.animation[37] += 1 * (g_csgo.m_globals->m_frametime * 5);
			g_visuals.animation[37] = std::clamp(g_visuals.animation[37], 0.35f, 1.f);
			SetAlpha(g_visuals.animation[37]);
		}
