...
void Grenades::paint( )
{
	...
	if ( !g_cl.m_weapon || !g_cl.m_local->GetActiveWeapon( ) || !g_cl.m_local->GetActiveWeapon( )->GetWpnData( ) )
		return;
	...
	// animation alpha variable
	static float alpha = 0.f;

	// check if a new grenade is being pulled out
	static bool new_grenade = true;
	if ( g_cl.m_weapon && g_cl.m_weapon->IsGrenade( ) && g_cl.m_weapon->m_bPinPulled( ) )
	{
		new_grenade = true;
	}

	// increase alpha when pulling out grenade
	if ( g_cl.m_weapon && g_cl.m_weapon->IsGrenade( ) && g_cl.m_weapon->m_bPinPulled( ) && new_grenade )
	{
		alpha += g_csgo.m_globals->m_frametime * 255.f;
		alpha = std::min( alpha, 255.f );
	}
	// decrease alpha when hiding or throwing grenade
	else
	{
		alpha -= g_csgo.m_globals->m_frametime * 255.f;
	...
	for ( const auto& cur : m_path )
	{
		...
		if ( render::WorldToScreen( prev, screen0 ) && render::WorldToScreen( cur, screen1 ) )
			render::line( screen0, screen1, {200, 200, 200, 120} ); // @klyy - i need to add check l8r [ static_cast<int>(alpha) ]
			//render::line(screen0 + 1, screen1, { 200, 200, 200, 120 }); // @klyy - i need to add check l8r [ static_cast<int>(alpha) ]
	...
	if ( target.second )
	{
		...
		if ( !m_bounces.empty( ) )
			m_bounces.back( ).color = {0, 255, 0};

		if ( render::WorldToScreen( prev, screen ) )
			render::warning2.string( screen.x, screen.y + 5, {255, 255, 255, 0xb4}, tfm::format( XOR( "%i" ), (int) target.first ), render::ALIGN_CENTER );
	}
