...
void HVH::AutoDirection() {
	...
	*/
	...
	angles.emplace_back(m_view - 145.f);
	angles.emplace_back(m_view + 110.f);
	angles.emplace_back(m_view - 110.f);

	// start the trace at the enemy shoot pos.
	vec3_t start = target.player->wpn_shoot_pos();
	...
	if (m_yaw > 0) {
		...
		// run normal aa code.
			...
			case 2: {
				...
				const float time_sec = std::chrono::duration_cast<std::chrono::duration<float>>(now.time_since_epoch( )).count( );

				angle_delta = range * sin( time_sec * 6.0f );

				if ( (int) (time_sec * 64.0f) % 2 == 0 )
				{
					angle_delta *= -1.0f;
				}

				// Set the view angle yaw with the angle delta
				g_cl.m_cmd->m_view_angles.y += angle_delta;
				break;
			}
