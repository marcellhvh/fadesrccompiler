...
bool math::IntersectSegmentSphere(const vec3_t& vecRayOrigin, const vec3_t& vecRayDelta, const vec3_t& vecSphereCenter, float flRadius) {
...
bool math::IntersectSegmentCapsule(const vec3_t& start, const vec3_t& end, const vec3_t& min, const vec3_t& max, float radius) {
	vec3_t d = max - min, m = start - min, n = end - start;
	float md = m.dot(d);
	float nd = n.dot(d);
	float dd = d.dot(d);
...
void math::VectorAngles1(const vec3_t& vecForward, vec3_t& vecAngles)
{
	vec3_t vecView;
...
void math::AngleVectors1(const vec3_t& angles, vec3_t* forward)
{
	float sp, sy, cp, cy;
