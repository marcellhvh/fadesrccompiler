...
public:
	...
	__forceinline EHANDLE& m_hViewModel() {
		return get< EHANDLE >(g_entoffsets.m_hViewModel);
	}
...
public:
	...
	__forceinline void ModifyEyePosition(CCSGOPlayerAnimState* animstate, vec3_t* pos, matrix3x4_t* matrix) {
		if (!animstate || !animstate->m_player)
			return;

		if (animstate->m_landing || animstate->m_player->m_flDuckAmount() != 0.f) /// in hitground
		{
	...
	__forceinline vec3_t GetShootPosition() {
		...
		vec3_t pos = wpn_shoot_pos();
		matrix3x4_t* bone_matrix = nullptr;

		if (*reinterpret_cast <int32_t*> (uintptr_t(this) + 0x39E1)) {
			auto v3 = m_PlayerAnimState();
			if (v3) {
				if (this->SetupBones(bone_matrix, 128, BONE_USED_BY_ANYTHING, g_csgo.m_globals->m_curtime)) {
					if(bone_matrix)
						ModifyEyePosition(v3, &pos, bone_matrix);
				}
			}
		}
...
public:
	...
	__forceinline bool IsGrenade() {
		return (GetWpnData()->m_weapon_type == WEAPONTYPE_GRENADE && m_iItemDefinitionIndex() != ZEUS && GetWpnData()->m_weapon_type != WEAPONTYPE_KNIFE);
	}
