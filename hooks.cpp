...
void __fastcall Hooks::hkVoiceData(void* msg) {
	...
	if (g_menu.main.misc.debug_voice.get()) {
		player_info_t player_info;

		if (g_csgo.m_engine->GetPlayerInfo(sender_index, &player_info))
			g_cl.print(tfm::format(XOR("hkVoiceData: player: %i | low: %i | high: %i | sequence_bytes: %i | yaw: %i \n"), player_info.m_name, m->xuid_low, m->xuid_high, m->sequence_bytes, m->section_number ));
	}

	// kaaba by vmexit
	if (m->xuid_low == 43955 || m->xuid_low == 43969 || m->xuid_low == 43803 || m->sequence_bytes == -858993664) {
		player_info_t player_info;
		...
		if (g_csgo.m_engine->GetPlayerInfo(sender_index, &player_info)) {
			...
			if (data) {
				data->m_kaaba_angle = math::NormalizedAngle(m->section_number);
			}
		}
	}

	// dopium
	if (m->xuid_low == 1338 && m->xuid_high == 1172013056 && m->sequence_bytes == -918206803) {
		player_info_t player_info;

		if (g_csgo.m_engine->GetPlayerInfo(sender_index, &player_info)) {
			g_cl.dopium_users.push_back(player_info.m_user_id);
		}
	}

	//vader
	if (!strcmp(packet->cheat_name, XOR("vader.techballs")) || !strcmp(packet->cheat_name, XOR("vader.techbetaballs"))
		|| !strcmp(packet->cheat_name, XOR("vader.techdevballs")) || !strcmp(packet->cheat_name, XOR("vader.tech"))
		|| !strcmp(packet->cheat_name, XOR("vader.tech2"))) {
		player_info_t player_info;

		if (g_csgo.m_engine->GetPlayerInfo(sender_index, &player_info)) {
			g_cl.vader_users.push_back(player_info.m_user_id);
		}
	}

	// fade
	if (!strcmp(packet->cheat_name, XOR("fade.tapping")))
	{
		player_info_t player_info;

		if (g_csgo.m_engine->GetPlayerInfo(sender_index, &player_info)) {
			g_cl.fade_users.push_back(player_info.m_user_id);
		}
	}

	// cheese beta
	if (m->sequence_bytes == 321420420) {
		player_info_t player_info;

		if (g_csgo.m_engine->GetPlayerInfo(sender_index, &player_info)) {
			g_cl.cheese_beta.push_back(player_info.m_user_id);
		}
	}

	// unkown
	if ( m->xuid_low == 1886611819 && m->sequence_bytes == 114 )
	{
		player_info_t player_info;

		if ( g_csgo.m_engine->GetPlayerInfo( sender_index, &player_info ) )
		{
			g_cl.fake_hack.push_back( player_info.m_user_id );
		}
	}
...
FnInterpolateServerEntities oInterpolateServerEntities;
void __fastcall InterpolateServerEntities() {
	...
	for (int i = 1; i <= g_csgo.m_globals->m_max_clients; ++i) {
		const auto pEntity = (Player*)g_csgo.m_entlist->GetClientEntity(i);
		if (!pEntity)
			continue;

		if (pEntity->index() == g_cl.m_local->index())
			continue;
...
void Hooks::init() {
	...
	const auto dwInterpolateServerEntities = (DWORD)pattern::find(g_csgo.m_client_dll, XOR("55 8B EC 83 EC 1C 8B 0D ? ? ? ? 53 56"));
	MH_CreateHook((byte*)dwInterpolateServerEntities, InterpolateServerEntities, reinterpret_cast<void**>(&oInterpolateServerEntities));
